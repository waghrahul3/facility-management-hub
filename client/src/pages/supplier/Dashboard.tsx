...
export default function SupplierDashboard() {
  ...
  if (!data || !week) return <LoadingScreen label="Loading supplier overview…" />;
    ...
    <div>
      <div className="mb-6">
        <h1 className="font-display text-2xl font-bold text-field-900">
          {user?.supplierName ?? "Supplier"} Dashboard
        </h1>
        <p className="mt-1 text-sm text-field-500">
          Week of {fmtDate(data.weekStart)} — collect and distribute on Sunday
        </p>
      </div>

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Drops this week" value={week.totalDrops} tone="green" icon={<span>🚛</span>} />
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <Card title="This week at a glance" subtitle={`Worker earnings − drop rent = net payment`}>
          <div className="space-y-3">
            <div className="flex justify-between text-sm">
              <span className="text-field-500">Total worker earnings</span>
              <span className="font-semibold"><Money value={week.totalWorkerEarnings} /></span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-field-500">Total rent charges ({week.totalDrops} drops)</span>
            ...
            <div className="flex justify-between border-t border-field-200 pt-3 text-base">
              <span className="font-semibold text-field-800">Net payment to collect</span>
              <span className="font-bold text-onion-800"><Money value={week.netPayment} /></span>
            </div>
          </div>
          <div className="mt-4">
            <Link to="/supplier/payments">
              <Button variant="success" className="w-full">Collect & distribute payment</Button>
            </Link>
          </div>
        </Card>

        <Card title="Your tolis this week" subtitle="Earnings per toli leader you dropped">
          {week.summaries.length === 0 ? (
            <EmptyState title="No toli summaries yet" hint="Work entries appear once approved by the facility" />
          ) : (
