...
export default function FacilitiesPage() {
  ...
  async function remove(f: Facility) {
    if (!confirm(`Delete facility "${f.name}"? This removes its data.`)) return;
  ...
  return (
    <div>
      <PageHeader
        title="Facilities"
        subtitle="Manage all onion processing facilities in the network"
        action={<Button onClick={openCreate}>+ New facility</Button>}
      />

      {!facilities ? (
        <LoadingScreen />
      ) : facilities.length === 0 ? (
        <Card><EmptyState title="No facilities yet" hint="Create your first facility" /></Card>
      ) : (
        <Card>
          <Table
            head={["Facility", "Location", "Capacity", "Company", "Admin", "Status", "Actions"]}
            empty={null}
          >
            {facilities.map((f) => (
              <tr key={f.id} className="hover:bg-field-50/50">
                ...
                <Td>{f.capacity ?? 0} workers</Td>
                <Td>
                  {f.company ? (
                    <Badge tone="blue">{f.company.name}</Badge>
                  ) : (
                    <span className="text-field-400">Standalone</span>
                  )}
                </Td>
                <Td>{f.admin ? f.admin.name : <span className="text-field-400">—</span>}</Td>
                <Td>
                  {f.is_active ? <Badge tone="green">Active</Badge> : <Badge tone="red">Inactive</Badge>}
                </Td>
                <Td>
                  <div className="flex gap-2">
                    ...
                    <Button variant="danger" size="sm" onClick={() => remove(f)}>Delete</Button>
                  </div>
                </Td>
              </tr>
            ))}
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editing ? "Edit facility" : "New facility"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Field label="Company (optional)" hint="One company can own multiple facilities">
            <SearchableSelect
              value={form.companyId}
              onChange={(v) => setForm({ ...form, companyId: v })}
              options={companies.map((c) => ({ value: c.id, label: c.name }))}
              placeholder="Standalone facility (no company)"
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button type="submit" loading={busy}>{editing ? "Save changes" : "Create facility"}</Button>
          </div>
        </form>
      </Modal>
    </div>
