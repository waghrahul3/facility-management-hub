...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!existing) throw notFound("Company admin not found");
    if (existing.role !== "COMPANY_ADMIN") throw badRequest("User is not a company admin");
    ...
    return res.json({ companyAdmin: updated });
  })
);
